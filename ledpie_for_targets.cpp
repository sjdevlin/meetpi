...
#define ANGLE_SPREAD 20  // Gives 40 degree window for each speaker
...
void capture_energy_level_at_location() {
  ...
  // check how far away it is from a known source
    int angle_diff = participant[i].angle - angle_xy;
    angle_diff += (angle_diff>180) ? -360 : (angle_diff<-180) ? 360 : 0;
    if (abs(angle_diff) < ANGLE_SPREAD) {
      ++num_matches;
      person_speaking = i;}
      // we can here start to average out the location for a speaker
}
...
// add to existing participant
     ...
     participant[person_speaking].num_turns += (participant[person_speaking].was_talking) ? 0 : 1;
